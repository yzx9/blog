...
含有 $n$ 个命题变元的合取式中，若每个命题变元与其否定不同时出现，而二者之一必出现且仅出现一次，这样的合取式称为**极小项**。

- 每个极小项在它的 $2^n$ 个赋值中，仅有一个是成真赋值。
- 含有 $n$ 个命题变元的公式，其不同的极小项的个数均为 $2^n$ 个，可以表示为 $m_0, m_1 , \dots ,m_{2^n-1}$。
...
含有 $n$ 个命题变元的析取式中，若每个命题变元与其否定不同时出现，而二者之一必出现且仅出现一次，这样的析取式称为**极大项**。

- 每个极大项在它的 $2^n$ 个赋值中，仅有一个是成假赋值。
- 含有 $n$ 个命题变元的公式，其不同的极大项的个数均为 $2^n$ 个，可以表示为 $M_0, M_1 , \dots ,M_{2^n-1}$
